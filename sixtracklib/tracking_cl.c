...
tracking_cl* tracking_cl_prepare(tracking_elements *elements, tracking_beam *beam, unsigned int nturn, int track_by_turn, int track_by_element) {
    ...
    cl_program program = build_program(context, dev, Kernel_File, build_options);
    if(program == NULL) {
        return NULL;
    }
    ...
    cl_mem elements_buffer = clCreateBuffer(context, CL_MEM_READ_ONLY | CL_MEM_COPY_HOST_PTR, elements->te_head * sizeof(value_t), elements->te_data, &err);
    Check_Failure("Failed to create OpenCL buffer for tracking_cl elements data.");
    cl_mem offsets_buffer = clCreateBuffer(context, CL_MEM_READ_ONLY | CL_MEM_COPY_HOST_PTR, (elements->te_offsets[0]+1) * sizeof(int), elements->te_offsets, &err);
    Check_Failure("Failed to create OpenCL buffer for tracking_cl elements offsets.");
    cl_mem beam_buffer = clCreateBuffer(context, CL_MEM_READ_WRITE | CL_MEM_COPY_HOST_PTR, beam->b_npart * sizeof(Particle), beam->b_particles, &err);
    Check_Failure("Failed to create OpenCL buffer for tracking_cl beam");
    tracking_beam *beam_by_turn = NULL;
    cl_mem beam_by_turn_buffer = NULL;
    tracking_beam *beam_by_element = NULL;
    cl_mem beam_by_element_buffer = NULL;
    if(track_by_turn) {
        beam_by_turn = (tracking_beam *)malloc(sizeof(tracking_beam));
        if(beam_by_turn == NULL) {
            perror("Failed to allocate tracking_cl beam by turn structure");
            return NULL;
        }
        beam_by_turn->b_npart = nturn * beam->b_npart;
        beam_by_turn->b_particles = (Particle *)malloc(beam_by_turn->b_npart * sizeof(Particle));
        if(beam_by_turn->b_particles == NULL) {
            perror("Failed to allocate tracking_cl beam by turn particles");
            return NULL;
        }
        beam_by_turn_buffer = clCreateBuffer(context, CL_MEM_WRITE_ONLY, beam_by_turn->b_npart * sizeof(Particle), beam_by_turn->b_particles, &err);
        Check_Failure("Failed to create OpenCL buffer for beam by turn particles.");
    }
    if(track_by_element) {
        beam_by_element = (tracking_beam *)malloc(sizeof(tracking_beam));
        if(beam_by_element == NULL) {
            perror("Failed to allocate tracking_cl beam by element structure");
            return NULL;
        }
        beam_by_element->b_npart = elements->te_offsets[0] * nturn * beam->b_npart;
        beam_by_element->b_particles = (Particle *)malloc(beam_by_element->b_npart * sizeof(Particle));
        if(beam_by_element->b_particles == NULL) {
            perror("Failed to allocate tracking_cl beam by element particles");
            return NULL;
        }
        beam_by_element_buffer = clCreateBuffer(context, CL_MEM_WRITE_ONLY, beam_by_element->b_npart * sizeof(Particle), beam_by_element->b_particles, &err);
        Check_Failure("Failed to create OpenCL buffer for beam by turn particles.");
    }
    int i = 0;
    ...
    if(track_by_turn) {
        err = clSetKernelArg(kernel, i++, sizeof(cl_mem), &beam_by_turn_buffer);
        Check_Failure("Failed to set argument of OpenCl kernel: particles_by_turn");
    }
    if(track_by_element) {
        err = clSetKernelArg(kernel, i++, sizeof(cl_mem), &beam_by_element_buffer);
        Check_Failure("Failed to set argument of OpenCl kernel: particles_by_element");
    }
    tracking_cl *track = (tracking_cl *)malloc(sizeof(tracking_cl));
    if(track == NULL) {
...
cl_int tracking_cl_execute(tracking_cl *track) {
    cl_int err;
    err = clEnqueueNDRangeKernel(track->t_queue, track->t_kernel, 1, NULL, &(track->t_beam->b_npart), NULL, 0, NULL, NULL);
    if(err < 0) {
...
cl_int tracking_cl_read(tracking_cl *track) {
    cl_int err;
    err = clEnqueueReadBuffer(track->t_queue, track->t_beam_buffer, CL_TRUE, 0, track->t_beam->b_npart * sizeof(Particle), track->t_beam->b_particles, 0, NULL, NULL);
    if(err < 0) {
        perror("Failed to read track_cl beam buffer");
        return err;
    }
    if(track->t_beam_by_turn_buffer != NULL && track->t_beam_by_turn != NULL) {
        err = clEnqueueReadBuffer(track->t_queue, track->t_beam_by_turn_buffer, CL_TRUE, 0, sizeof(Particle), track->t_beam_by_turn, 0, NULL, NULL);
        if(err < 0) {
            perror("Failed to read tracking_cl beam by turn buffer");
        }
    }
    if(track->t_beam_by_element_buffer != NULL && track->t_beam_by_element != NULL) {
        err = clEnqueueReadBuffer(track->t_queue, track->t_beam_by_element_buffer, CL_TRUE, 0, sizeof(Particle), track->t_beam_by_element, 0, NULL, NULL);
        if(err < 0) {
            perror("Failed to read tracking_cl beam by element buffer");
        }
    }
    return err;
}
