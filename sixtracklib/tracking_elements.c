...
{
	struct tracking_elements *elements =
	    (struct tracking_elements *)
	    malloc(sizeof(struct tracking_elements));
	if (elements == NULL) {
	...
	unsigned int *offsets =
	    (unsigned int *)malloc(sizeof(unsigned int) * (size + 1));
	if (offsets == NULL) {
			       ...
			       unsigned int n)
{
	...
	value_t *ndata =
	    (value_t *) realloc(elements->te_data, sizeof(value_t) * nsize);
	if (ndata == NULL) {
		perror("Failed to reallocate tracking_cl elements data.");
		return;
	}
	unsigned int *noffsets =
	    (unsigned int *)realloc(elements->te_offsets,
				    sizeof(unsigned int) * (nsize + 1));
	if (noffsets == NULL) {
